...
mixed findrate(string zip, string weight, mapping|void options, object id){
  ...
  query_variables+=([
	...
	"ResponseType": "application/x-ups-rrs",
	"ActionCode": "3",		// Show rates for chosen method.
	"ResidentialInd": "1",
	"PackagingType": "00"
	]);
  ...
  mixed result=Protocols.HTTP.post_url_nice(ECURL, query_variables);
...
Config.write_section(id->misc->ivend->this_object->query("configdir")+
  CONFIG->global->config, "ShippingType_" + vars->type,
 ...
 retval+="Oversize Indicator: <SELECT NAME=\"oversize_field\">\n"
...
if(r[0]->calctype=="T") {  // We calculate everything as if it were in a big box.
  ...
  rate=findrate((string)zip, (string)w);
